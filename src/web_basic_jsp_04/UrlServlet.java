...
public class UrlServlet extends HttpServlet {
	...
	private void process(HttpServletRequest request, HttpServletResponse response)
		...
		String homep = request.getParameter("homepage");
		...
		String [] mails = request.getParameterValues("mailing");
		...
		out.printf("id %s<br> password %s<br>", id, 	(pw1.equals(pw2)? "일치": "불일치"));
		out.printf("name %s<br>", name);
		out.printf("email %s<br>", mail);
		out.printf("phone %s<br>", phone);
		out.printf("homepage %s<br>", homep); 
		...
		out.printf("subject  "+ t );
		out.printf("<br>");
		
		
		if(mails ==null) {
			out.printf("선택한 수강분야가 없음 <br>");
		}else {
		for(String mailing : mails) {
			if(mailing.equals("news")) {
					out.printf("메일링 =%s<br> ", " 해외단신" );
